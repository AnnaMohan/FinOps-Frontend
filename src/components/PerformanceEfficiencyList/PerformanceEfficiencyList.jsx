...
import UpdatePerformanceModal from "../UpdateModal/UpdatePerformanceModal";
...
const PerformanceEfficiencyList = () => {
  ...
  return (
    <div className="ps-3 pe-3 pt-3">
      ...
      {isLoading ? (
        ...
        <Table responsive className="table table-striped table-primary w-auto">
          ...
          <tbody className="text-wrap">
            {paginate(performance, currentPage, itemsPerPage).map((policy) => (
                    ...
                    {isDeployLoader && policyId === policy.policy_id ? (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          deployPerformancePolicyFile(policy.policy_id);
                        }}
                      >
                        Deploy
                      </button>
                ...
                <td className="align-middle">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setPolicyId(policy.policy_id);
                      getPerformanceOnePolicy(policy.policy_id);
                    }}
                  >
                    {/* Update */}
                    Edit
        ...
        <Pagination className="mt-3">
            {Array.from({ length: Math.ceil(performance.length / itemsPerPage) }).map(
              (_, index) => (
                <Pagination.Item
                  key={index}
      ...
      {showModal && performanceDetails && (
        <UpdatePerformanceModal
          cancel={cancel}
          policyName={policyName}
          performanceDetails={performanceDetails}
          policyId={policyId}
          updatePerformancePolicyFile={updatePerformancePolicyFile}
          updatePerformanceMsg={updatePerformanceMsg}
...
export default PerformanceEfficiencyList;
