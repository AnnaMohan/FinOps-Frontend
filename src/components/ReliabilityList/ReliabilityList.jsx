...
import UpdateReliabilityModal from "../UpdateModal/UpdateReliabilityModal";
...
const ReliabilityList = () => {
  ...
  return (
    <div className="ps-3 pe-3 pt-3">
      ...
      {isLoading ? (
        ...
        <Table responsive className="table table-striped table-primary w-auto">
          ...
          <tbody className="text-wrap">
            {paginate(reliability, currentPage, itemsPerPage).map((policy) => (
                    ...
                    {isDeployLoader && policyId === policy.policy_id ? (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          deployReliabilityPolicyFile(policy.policy_id);
                        }}
                      >
                        Deploy
                      </button>
                ...
                <td className="align-middle">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setPolicyId(policy.policy_id);
                      getReliabilityOnePolicy(policy.policy_id);
                    }}
                  >
                    {/* Update */}
                    Edit
        ...
        <Pagination className="mt-3">
            {Array.from({ length: Math.ceil(reliability.length / itemsPerPage) }).map(
              (_, index) => (
                <Pagination.Item
                  key={index}
      ...
      {showModal && reliabilityDetails && (
        <UpdateReliabilityModal
          cancel={cancel}
          policyName={policyName}
          reliabilityDetails={reliabilityDetails}
          policyId={policyId}
          updateReliabilityPolicyFile={updateReliabilityPolicyFile}
          updateReliabilityMsg={updateReliabilityMsg}
